 ...
 *   px4_ap1_pwm_aux_out.cpp 
 *
 */

#define S_FUNCTION_NAME                px4_ap1_pwm_aux_out
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	ssSetNumSFcnParams(S, MAX_AUX_OUT_PORTS + 2); // 2 additional parameters for pwm rates
  ...
  int cntPorts = 0;
  for(int i = 0; i < MAX_AUX_OUT_PORTS; i++)
  {
  ...
  if (!ssSetNumInputPorts(S, cntPorts))
    return;
 
  for(int i = 0; i < cntPorts; i++)
  {
