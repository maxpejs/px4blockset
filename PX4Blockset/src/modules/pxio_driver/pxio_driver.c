...
void pxio_driver_init(void)
{
	if(module_state == DISABLE)
	{
		...
		{
			px4debug(eCOMMITF, "pxio drv HAL_UART_Init error! \r\n");
			error_handler(0);
		}
		...
		{
			px4debug(eCOMMITF, "pxio drv HAL_DMA_Init tx error! \r\n");
			error_handler(0);
		}
		...
		{
			px4debug(eCOMMITF, "pxio drv HAL_DMA_Init rx error! \r\n");
			error_handler(0);
		}
		...
		module_state = ENABLE;
		px4debug(eCOMMITF, "pxio_driver init ok\r\n");
		...
		if (tries == 0)
		{
			px4debug(eCOMMITF, "no communication to pxio\r\n");
		}
		else
		{
			px4debug(eCOMMITF, "communication with pxio is established\r\n");
		}
	}
}
...
int32_t pxio_driver_reg_mod(uint8_t page, uint8_t offset, uint16_t clearbits, uint16_t setbits)
{
	...
	if (ret != SUCCESS)
	{
		px4debug(eCOMMITF, "reg mod get err\r\n");
		return ret;
	}
	...
	if (ret != SUCCESS)
	{
		px4debug(eCOMMITF, "reg mod set err\r\n");
		return ret;
	}
	return ret; 
}
...
int32_t read(uint8_t page, uint8_t offset, uint16_t * values, uint8_t count)
{
	...
	if (result == SUCCESS)
	{
		if (PKT_CODE(dmaRxBuffer) == PKT_CODE_ERROR)
		{
			px4debug(eCOMMITF, "pck code err\r\n");
			result = ERROR;
		}
		else if (PKT_COUNT(dmaRxBuffer) != count) /* compare register counts */
		{
			px4debug(eCOMMITF, "recv cnt err\r\n");
			result = ERROR;
		}
...
int32_t com_complete()
{
	...
	//	{
	//		px4debug(eCOMMITF, "dma timeout on start \r\n");
	//	}
	//
